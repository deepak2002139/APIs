...
//   const i=find((joke)=>joke.id===id);
//   res.json(i);
// });

app.get("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const foundJoke = jokes.find((joke) => jokes.id === id);
  res.json(foundJoke);
});
...
app.get("/filter",(req,res)=>{
  ...
  const x=jokes.filter((joke)=> jokeType===type);
  res.json(x);
});
...
app.put("/jokes/id:",(req,res)=>{
  ...
  const ii=jokes.findIndex((joke)=>joke.id===idd)
  jokes[ii]=replace;
  res.json(replace);
});
...
app.patch("jokes/:id",(req,res)=>
{
  ...
  let si=req.findIndex((joke)=>joke.id);
  jokes[si]=replacementJoke;
...
app.delete("jokes/:id",(req,res)=>{
  let id=parseInt(req.params.id);
  let si=jokes.findIndex((joke)=>joke.id===id);
  if(searchIndex>-1)
  {
    jokes.splice(si,1);
    res.sendStatus(200);
...
app.delete("/all",(req,res)=>{
  ...
  {
    res.status(404)
    res.json({error:`You are not authorised to perform this action.`})
  }
});
